...
`QueryColumnFilter` is not listed — it supports `=`, `!=`, `>`, `<`, `>=`, `<=` with the same type restrictions as those operators. Both columns must have compatible types (same type or same type family: numeric `int`/`decimal`, temporal `date`/`timestamp`). Array columns are not allowed in `QueryColumnFilter`.
